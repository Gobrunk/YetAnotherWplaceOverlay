...
export class WindowMain extends Overlay {
        ...
        this.addDiv({ id: this.windowId, class: 'yawo-window yawo-compact', style: 'top: 10px; left: unset; right: 75px;' })
            ...
            .addDiv({ class: 'yawo-content' })
                ...
                .addDiv({ class: 'yawo-col' })
                    .addDiv({ class: 'yawo-col' })
                        .addButton({ class: 'yawo-chrome-btn yawo-jump-btn', style: 'margin-top: 0;', innerHTML: '<svg viewBox="0 0 4 6"><path d="M.5,3.4A2,2 0 1 1 3.5,3.4L2,6"/><circle cx="2" cy="2" r=".7" fill="#fff"/></svg>' }, (overlay, btn) => {
                            btn.onclick = () => {
                                const coords = overlay.apiManager?.lastClickCoords;
                                if (coords?.[0]) {
                                    overlay.setElementContent('yawo-tile-x',  coords?.[0] || '');
                                    overlay.setElementContent('yawo-tile-y',  coords?.[1] || '');
                                    overlay.setElementContent('yawo-pixel-x', coords?.[2] || '');
                                    overlay.setElementContent('yawo-pixel-y', coords?.[3] || '');
                                } else {
                                    overlay.setError('Coordinates are malformed! Did you try clicking on the canvas first?');
                                }
                            };
                        }).up()
                        .addInput({ type: 'number', id: 'yawo-tile-x',  class: 'yawo-coord-input', placeholder: 'Tl X', min: 0, max: 2047, step: 1, required: true }, (overlay, inp) => {
                            inp.addEventListener('paste', e => this.#handleCoordPaste(overlay, inp, e));
                        }).up()
                        .addInput({ type: 'number', id: 'yawo-tile-y',  class: 'yawo-coord-input', placeholder: 'Tl Y', min: 0, max: 2047, step: 1, required: true }, (overlay, inp) => {
                            inp.addEventListener('paste', e => this.#handleCoordPaste(overlay, inp, e));
                        }).up()
                        .addInput({ type: 'number', id: 'yawo-pixel-x', class: 'yawo-coord-input', placeholder: 'Px X', min: 0, max: 2047, step: 1, required: true }, (overlay, inp) => {
                            inp.addEventListener('paste', e => this.#handleCoordPaste(overlay, inp, e));
                        }).up()
                        .addInput({ type: 'number', id: 'yawo-pixel-y', class: 'yawo-coord-input', placeholder: 'Px Y', min: 0, max: 2047, step: 1, required: true }, (overlay, inp) => {
                            inp.addEventListener('paste', e => this.#handleCoordPaste(overlay, inp, e));
                        }).up()
                    .up()
                    .addDiv({ class: 'yawo-col' })
                        .addFileInput({ class: 'yawo-file-upload', textContent: 'Upload Template', accept: 'image/png, image/jpeg, image/webp, image/bmp, image/gif' }).up()
                    .up()
                    .addDiv({ class: 'yawo-col yawo-wrap yawo-action-btns' })
                        ...
                        .addButton({ innerHTML: '🖼️ Create' }, (overlay, btn) => {
                            btn.onclick = () => {
                                ...
                                const tileXInp  = document.querySelector('#yawo-tile-x');
                                const tileYInp  = document.querySelector('#yawo-tile-y');
                                const pixelXInp = document.querySelector('#yawo-pixel-x');
                                const pixelYInp = document.querySelector('#yawo-pixel-y');
    ...
    async #handleCoordPaste(overlay, inputEl, event) {
        ...
        if (nums.length === 2 && inputEl.id === 'yawo-pixel-x') {
            overlay.setElementContent('yawo-pixel-x', nums[0] || '');
            overlay.setElementContent('yawo-pixel-y', nums[1] || '');
        } else if (nums.length === 1) {
            overlay.setElementContent(inputEl.id, nums[0] || '');
        } else {
            overlay.setElementContent('yawo-tile-x',  nums[0] || '');
            overlay.setElementContent('yawo-tile-y',  nums[1] || '');
            overlay.setElementContent('yawo-pixel-x', nums[2] || '');
            overlay.setElementContent('yawo-pixel-y', nums[3] || '');
        }
    }
}
